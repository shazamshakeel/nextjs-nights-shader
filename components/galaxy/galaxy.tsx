...
// Mix amount toward black applied in the final compose pass. 0 = scene at
// full brightness; STARS_SCROLL_FADE_MAX = stars dimmed to (1 - max) of
// their normal brightness. Driven by the same scroll handler as
// `logoInfluence` so the logo fade and stars dim move in lockstep — on
// mobile the entire celestial layer recedes as the user scrolls past the
// logo into the main content.
const starsScrollDim = uniform(0);
const STARS_SCROLL_FADE_MAX = 0.5;
...
export function Galaxy(): JSX.Element {
  ...
  useEffect(() => {
    ...
    const updateLogoLayout = (): void => {
      ...
      // Y is flipped because canvas UV has y=0 at the bottom while DOM
      // rects measure from the top.
      logoCenter.value.set(cx / containerW, 1 - cy / containerH);
    ...
    const updateLogoInfluence = (): void => {
      ...
      starsScrollDim.value = t * STARS_SCROLL_FADE_MAX;
    };
    ...
    // mousePos = current mouse UV (relative to the canvas container).
    ...
    const animate = (): void => {
      ...
      const elapsedMs =
        revealStart === null ? 0 : performance.now() - revealStart;
      ...
      // to its target (10.0); logoMin rides the same instantaneous rate and
      // naturally clamps at its much smaller target (0.02), so the two pass
      // through 0.02 *at the same moment* before logoMax keeps growing.
      ...
      // logoWhiteStrength reveals linearly — constant rate of change, no
      // curve. Starts after LINE_REVEAL_DELAY_MS so the mask brightening
      // can establish first.
      const tWhite = Math.min(
        Math.max(elapsedMs - LINE_REVEAL_DELAY_MS, 0) / LINE_REVEAL_DURATION_MS,
        1,
      );
      const easedWhite = tWhite;
      logoWhiteStrength.value = revealTargets.logoWhiteStrength * easedWhite;
    ...
    Promise.all([
      ...
      const maskMat = new THREE.NodeMaterial();
      {
        ...
        const redCombined = logoSample.r
          .add(logoSample.b.mul(diagonalBoost))
          .clamp(0, 1);
      ...
      // and actually scrolls), the whole celestial layer recedes. At
  ...
  useEffect(() => {
    ...
    void import('lil-gui').then(({ default: GUI }) => {
      ...
      logoFolder
        .add(revealTargets, 'logoMinMul', 0, 5, 0.01)
        .name('min mul (red=0)');
      logoFolder
        .add(revealTargets, 'logoMaxMul', 0, 20, 0.01)
        .name('max mul (red=1)');
      logoFolder.add(logoCurve, 'value', 0.1, 6, 0.01).name('curve');
      logoFolder
        .add(revealTargets, 'logoWhiteStrength', 0, 10, 0.01)
        .name('line power');
